...
class RegistrationManager {
    ...
    async init() {
        ...
        try {
            ...
            this.showError('Сервис временно недоступен');
        }
    }
    ...
    setupEventListeners() {
        ...
        // Отправка формы
        document.getElementById('registerForm').addEventListener('submit', (e) => this.submitForm(e));
    ...
    validateStep2() {
        ...
        if (!phone) {
            ...
            errors.push('Введите корректный номер телефона Беларуси');
        }
    ...
    saveStepData(step) {
        switch(step) {
            case 1:
                ...
                break;
            case 2:
                this.userData.phone = '+375 ' + (document.getElementById('phone')?.value || '');
    ...
    generateUsername() {
        ...
        // Если активен ручной режим, обновляем оба поля
        const manualSection = document.getElementById('manualUsernameSection');
        if (manualSection && manualSection.style.display !== 'none') {
            // Обновляем ручное поле
            const manualUsernameField = document.getElementById('manualUsername');
    ...
    enableManualUsername() {
        ...
        // Переключаем фокус на ручной ввод
        setTimeout(() => {
            const manualUsernameField = document.getElementById('manualUsername');
            if (manualUsernameField) {
    ...
    validatePhone(phone) {
        ...
        if (input) {
            this.toggleFieldValidation(input, isValid, isValid ? 'Номер корректный' : 'Введите 9 цифр номера');
        }
        return isValid;
    }
    ...
    async submitForm(e) {
        ...
        try {
            ...
            if (!dataService) {
                throw new Error('Сервис данных недоступен');
            }
            
            // Формируем полное ФИО
            const fullName = this.userData.middleName ? 
            ...
            this.showSuccess('Регистрация успешна! Вы будете перенаправлены на страницу входа.');
            
            // Чистим форму и сбрасываем состояние
            setTimeout(() => {
            ...
            this.showError(error.message || 'Ошибка регистрации. Попробуйте еще раз.');
        } finally {
            const submitBtn = document.getElementById('submitRegistration');
            if (submitBtn) {
